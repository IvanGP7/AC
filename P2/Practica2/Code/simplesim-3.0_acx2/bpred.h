...
struct bpred_dir_t {
  ...
  union {
    ...
    struct {
      int l1size;			/* Cantidad de entradas del GBHR */ //SIEMPRE 1
      int l2size;			/* Numero de entradas del PHT */
      int shift_width;			/* Numero de bits del GBHR */
      int xor;				/* Siempre 0 ya que querremos concatenar para ser mas específicos */
      int *shiftregs;			/* Tabla GBHR con numero de entradas del l1size haremos malloc */
      unsigned char *l2table;		/* Nuemro de entradas del PHT haciendo malloc con l2size */
      unsigned char *Taken_pht;		/* Dirección del PHT Taken*/
      unsigned char *NotTaken_pht;	/* Dirección del PHT NOT Taken*/
      int size_pht2;
      int mascara_i;
      int mascara_g;
      unsigned char *contador;  /*Resultado del algoritmo de predicción*/
    } two;
  } config;
};
