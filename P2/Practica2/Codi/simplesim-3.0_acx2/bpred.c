	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPredNotTaken:
    ...
    //Creamos el caso del BPredCreate sabemos que l1size siempre será 1 y el xor será 0
  case BPredYAGS:
  ...
  switch (class) {
  case BPredYAGS: //añadimos nuestro caso
  case BPredComb:
  case BPred2Level:
  case BPred2bit:
    {
...
static void predictor_yags(struct bpred_dir_t *pred_dir, unsigned int l1size, unsigned int l2size, unsigned int shift_width, unsigned int xor){
  /*
    Para evitar errores de entrada filtramos todos los parametros
    l1size -> Numero de entradas del GBHR (siempre 1)
    l2size -> Numero de entradas del PHT principal [4, 16, 64, 256, 1024]
    shift_width -> Longitud del GBHR [1, 2, 3, 4, 5]
    xor -> Condición de XOR o Concatenación (siempre 0)
  */

  if(l1size != 1)
    fatal("El primer valor de entrada l1size '%d' tiene que ser 1.", l1size);
  if(l2size != 4 && l2size != 16 && l2size != 64 && l2size != 256 && l2size != 1024)
    fatal("El segundo valor de entrada l2size '%d' tiene que ser [4, 16, 64, 256, 1024]", l2size);
  if(shift_width < 1 || shift_width > 5)
    fatal("El tercer valor de entrada shift_width '%d' tiene que estar entre 1 y 5.", shift_width);
  if(xor != 0)
    fatal("El cuarto valor de entrada xor '%d' tiene que ser 0.", xor);

  // Inicializamos los datos sobre la estructura
  ...
  // Creamos la tabla de GBHR con 32 bits
  pred_dir->config.two.shiftregs = calloc(l1size, sizeof(int));
  if(!pred_dir->config.two.shiftregs)
    fatal("No se puede distribuir en memoria la tabla GBHR");
      // Inicializamos la tabla GBHR poniendo el número de bits shift_width a 1
  int bits_gbhr_inicializados = (1 << shift_width) - 1;
  pred_dir->config.two.shiftregs[0] = bits_gbhr_inicializados;

  // Creamos la tabla de PHT
  pred_dir->config.two.l2table = calloc(l2size, sizeof(unsigned char));
  if(!pred_dir->config.two.l2table)
    fatal("No se puede distribuir en memoria la tabla PHT");
      // Inicializamos los valores a 1
  for(int i = 0; i < l2size; i++){
    pred_dir->config.two.l2table[i] = 0x3;
  }

  // Creamos las tablas TakenPHT NotTakenPHT
  
      // El tamaño de esta tabla será (l2size / 8)
  if(l2size == 4)
    pred_dir->config.two.size_pht2 = 1;
  else
    pred_dir->config.two.size_pht2 = l2size >> 3;
      // Asignamos el tamaño requerido
  pred_dir->config.two.Taken_pht = calloc(pred_dir->config.two.size_pht2, sizeof(unsigned char));
  pred_dir->config.two.NotTaken_pht = calloc(pred_dir->config.two.size_pht2, sizeof(unsigned char));

  if(!pred_dir->config.two.Taken_pht)
    fatal("No se puede distribuir en memoria la tabla TakenPHT");

  if(!pred_dir->config.two.NotTaken_pht)
    fatal("No se puede distribuir en memeoria la tabla NotTakenPHT");
      // Inicializamos los 8 bits a 1 (6 tags || 2 contador)
  for(int i = 0; i < pred_dir->config.two.size_pht2; i++){
    pred_dir->config.two.Taken_pht[i] = 0xFF;
    pred_dir->config.two.NotTaken_pht[i] = 0xFF;
  }

  // Tablas inicializadas toca calcular los bits de 'c', 'g' y 'i'
  /*
  c -> log2(l2size)
  g -> shift_width
  i -> i = c -g (ya que c será la concatenación de i||g)
  */
  
      // Se ha calculado la mascara anteriormente cuando inicializamos
      // los bits correspondientes del shiftregs[0] a 1.
  pred_dir->config.two.mascara_g = bits_gbhr_inicializados;

      // Calculamos c haciedo el logaritmo en base 2, 
      // haciendo lsr de l2size hasta ser 0 (posición del bit más alto)
  int contador_log = l2size;
  int c = 0;
  while(contador_log > 1){
    contador_log >>= 1;
    c++;
  }

      // Ahora con los valores los restamos y creamos la mascara de i
  int valor_i = c - shift_width;
  pred_dir->config.two.mascara_i = (1 << valor_i) - 1;
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPredYAGS:
      ...
      /*
        Aqui tenemos que crear la estructura guardando espacios de memoria,
        y inicializar los valores de las tablas, los cuales serán 1 para empezar siempe con taken.

        Pasamos la estructura pred_dir y los parametros de entrada.
      */
     predictor_yags(pred_dir, l1size, l2size, shift_width, xor);
  ...
  FILE *stream)			/* output stream */
{
  ...
  case BPred2bit:
    ...
    // Generamos los parametros de salida tras la configuración
  case BPredYAGS:
    fprintf("pred_dir: %s, Entradas del PHT: %d, Entradas del T/NT PHT: %d, longitud GBHR, %d, %s XOR", 
    name, 
    pred_dir->config.two.l2size,
    pred_dir->config.two.size_pht2,
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  switch (pred->class)
    ...
    case BPred2Level:
      name = "bpred_2lev";
      break;
    case BPredYAGS: // Configuramos su nombre
      name = "bpred_yags";
      break;
    case BPred2bit:
      name = "bpred_bimod";
...
static char *algoritmo_prediccion_yags(struct bpred_dir_t *pred_dir, md_addr_t baddr){
    ...
    if(contador_pht){
      /*
        ...
        y obtener el tag almacenado del PHT 'tag_pht' y compararlo con los 6 ulitmos bits del PC 'tag'
      */
    ...
    // En caso de haber accedido a una de las tablas y no haber encontrado el tag devolvemos el salto origianl del PHT
    return p;
		 ...
		 md_addr_t baddr)		/* branch address */
{
  ...
  switch (pred_dir->class) {
    ...
    case BPredNotTaken:
      ...
      // Creamos nuestro algoritmo de predicción en base al manual, pasando por parametro 
      // la estructura creada y la dirección del salto (PC).
    case BPredYAGS:
      p = algoritmo_prediccion_yags(pred_dir, baddr);
    break;
    default:
      panic("bogus branch direction predictor class");
    }
...
/*------------------------------------------------------------------------------------------------*/
  /* Crear la actualización de toda la estructura del YAGS */

    if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND) &&
      (pred->class == BPredYAGS))
    {
      // tenemos que mover los bits del GBHR y añadir el resultado
      pred->dirpred.twolev->config.two.shiftregs[0] << 1;

      //miramos si el salto ha sido taken o no y lo actualizamos en el GBHR
      if(taken)
        pred->dirpred.twolev->config.two.shiftregs[0] | 1;
      
      //Obtenemos los 6 bits de menor peso de la dirección
      int direccion = baddr & 0x3F;

      // Actualizamos el contador saturado de la tabla PHT principal
      int bits_g = pred->dirpred.twolev->config.two.shiftregs[0] & pred->dirpred.twolev->config.two.mascara_g;
      int bits_i = baddr & pred->dirpred.twolev->config.two.mascara_i;
      int bits_c = (bits_i << pred->dirpred.twolev->config.two.shift_width) | bits_g;

      int contador = pred->dirpred.twolev->config.two.l2table[bits_c] & 0x3;
      if(taken && ( contador < 3 ))
        contador++;
      ...
      if(!taken && ( contador > 0 ))
        contador--;
            // Valor actualizado
      pred->dirpred.twolev->config.two.l2table[bits_c] = contador;

      // Actualizamos los contadores de las tablas T/NT PHT y añadimos la dirección.
      if(taken){
        contador = pred->dirpred.twolev->config.two.Taken_pht[bits_c % pred->dirpred.twolev->config.two.size_pht2] & 0x3;
        if(contador < 3)
          contador++;

        contador |= (direccion << 2);
        pred->dirpred.twolev->config.two.Taken_pht[bits_c % pred->dirpred.twolev->config.two.size_pht2] = contador;
      }
      else{
        contador = pred->dirpred.twolev->config.two.NotTaken_pht[bits_c % pred->dirpred.twolev->config.two.size_pht2] & 0x3;
        if(contador > 0)
          contador--;

        contador |= (direccion << 2);
        pred->dirpred.twolev->config.two.NotTaken_pht[bits_c % pred->dirpred.twolev->config.two.size_pht2] = contador;
      }
